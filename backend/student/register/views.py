...
class RegisterStartVerificationView(APIView):
    """
    API nhận dữ liệu đăng ký, kiểm tra email, SĐT, user_name tồn tại, 
    ...
    def post(self, request):
        ...
        user_name = data.get('name') # <-- Lấy user_name từ 'name' của form data
        
        if not email or not user_name:
            return Response({"success": False, "message": "Thiếu email hoặc tên người dùng."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            # 3. (MỚI) KIỂM TRA USER_NAME ĐÃ TỒN TẠI CHƯA
            # Giả định cột trong DB của bạn tên là 'user_name' như trong hình
            check_username = supabase.table(ACCOUNT_TABLE).select('user_name').eq('user_name', user_name).execute()
            if check_username.data:
                return Response({"success": False, "message": "Tên người dùng đã tồn tại trong hệ thống. Vui lòng nhập lại."}, 
                                status=status.HTTP_400_BAD_REQUEST)

            # 4. TẠO MÃ XÁC THỰC VÀ THỜI GIAN HẾT HẠN
            ...
            # 6. LƯU TẠM DỮ LIỆU ĐĂNG KÝ VÀ MÃ CODE
            ...
            # 7. TRẢ VỀ CÁC MÃ CODE CHO FRONTEND
            return Response({
                "success": True, 
