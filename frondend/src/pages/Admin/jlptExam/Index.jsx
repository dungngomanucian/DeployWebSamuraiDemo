...
import { getAllJlptExam } from '../../../api/admin/manageJlptExamService';
...
// COLUMN_CONFIG cho bảng Teacher
// COLUMN_CONFIG cho bảng jlpt_exams
const COLUMN_CONFIG = {
  'level_id': { // Hoặc tên cột tương ứng nếu có join để lấy tên level
    header: 'Cấp độ',
    type: 'text'
  },
  'title': {
  ...
  'total_duration': {
    ...
    type: 'number' // Để sắp xếp dạng số
  },
  'request_score': {
    header: 'Điểm yêu cầu',
    type: 'number' // Để sắp xếp dạng số
...
function Index() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const { data: apiResponse, error: apiError } = await getAllJlptExam(currentPage, pageSize);
